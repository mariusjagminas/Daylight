...
const getGeoData = async query => {
	...
	const response = await fetch(`${baseUrl}search?q=${encodedQuery}&maxRows=1&username=${process.env.USER}&type=json`);

	return await response.json();
};

export const getSunriseSunsetTimes = async (lat, lng) => {
	const response = await fetch(`${baseUrl}timezoneJSON?lat=${lat}&lng=${lng}&username=${process.env.USER}`);
